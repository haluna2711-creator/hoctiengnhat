...
function LuyenTapInner() {
  ...
  function handleScoreFinish(r: { correct: number; total: number }) {
    recordSession({ mode: mode as "mc" | "hiragana" | "kanji", level, correct: r.correct, total: r.total });
    setResult({ kind: "score", ...r });
    setStage("summary");
  }

  function handleFlashcardFinish(r: { known: number; total: number }) {
    recordSession({ mode: "flashcard", level, correct: r.known, total: r.total });
    setResult({ kind: "flashcard", ...r });
    setStage("summary");
  }

  function handleMatchFinish(r: { pairs: number; timeMs: number; mistakes: number }) {
    // Chưa có khái niệm "câu đúng/sai" rõ ràng ở chế độ ghép từ — coi
    // mỗi lần bấm sai là 1 lượt trừ điểm để vẫn có độ chính xác so
    // sánh được với các chế độ khác trên trang Đánh giá.
    const total = r.pairs + r.mistakes;
    recordSession({ mode: "match", level, correct: r.pairs, total });
  ...
  return (
    <div className="mx-auto max-w-3xl px-5 py-12 lg:max-w-4xl lg:px-8 lg:py-16">
      <h1 className="font-display text-3xl text-sumi lg:text-5xl">Luyện tập</h1>

      {stage === "setup" && (
        <div className="mt-6 space-y-8 lg:mt-10 lg:space-y-10">
          {errorMsg && (
            <p className="rounded-lg border border-beni/40 bg-beni/5 px-4 py-3 text-sm text-beni-deep lg:px-5 lg:py-4 lg:text-base">
              {errorMsg}
            </p>
          )}

          <div>
            <p className="mb-3 text-sm font-semibold text-sumi lg:mb-4 lg:text-lg">Cấp độ</p>
          ...
          <div>
            <p className="mb-3 text-sm font-semibold text-sumi lg:mb-4 lg:text-lg">Chế độ luyện tập</p>
            <div className="grid gap-3 sm:grid-cols-3 lg:gap-5">
              {MODES.map((m) => (
                <button
                  ...
                  className={`rounded-xl2 border p-4 text-left transition lg:p-6 ${
                    mode === m.value
                      ? "border-ai bg-ai/5"
                      : "border-line hover:border-ai/50"
                  }`}
                >
                  <span className="kanji-cell flex h-10 w-10 items-center justify-center rounded-sm font-jp text-lg text-ai lg:h-14 lg:w-14 lg:text-2xl">
                    {m.sample}
                  </span>
                  <p className="mt-2 font-display text-base text-sumi lg:mt-3 lg:text-xl">{m.title}</p>
                  <p className="mt-1 text-xs text-sumi-soft lg:mt-1.5 lg:text-sm">{m.desc}</p>
                </button>
              ))}
            </div>
          </div>
          ...
          {mode === "kanji" && (
            <div>
              <p className="mb-3 text-sm font-semibold text-sumi lg:mb-4 lg:text-lg">Chiều luyện viết kanji</p>
              <div className="flex flex-wrap gap-2 lg:gap-3">
          ...
          {mode === "flashcard" && (
            <div>
              <p className="mb-3 text-sm font-semibold text-sumi lg:mb-4 lg:text-lg">Chế độ ôn tập</p>
              <div className="flex flex-wrap gap-2 lg:gap-3">
                <Chip
              ...
              <p className="mt-2 text-xs text-sumi-soft lg:mt-3 lg:text-sm">
          ...
          {mode !== "flashcard" && (
            <div>
              <p className="mb-3 text-sm font-semibold text-sumi lg:mb-4 lg:text-lg">Đếm ngược 20 giây mỗi câu</p>
              <div className="flex flex-wrap gap-2 lg:gap-3">
          ...
          {(mode === "mc" || mode === "hiragana" || mode === "kanji") && (
            <div>
              <p className="mb-3 text-sm font-semibold text-sumi lg:mb-4 lg:text-lg">Tự động chuyển câu</p>
              <div className="flex flex-wrap gap-2 lg:gap-3">
          ...
          <div>
            <p className="mb-3 text-sm font-semibold text-sumi lg:mb-4 lg:text-lg">{countLabelFor(mode)}</p>
            <div className="flex flex-wrap gap-2 lg:gap-3">
          ...
          <button
            type="button"
            onClick={handleStart}
            className="rounded-full bg-ai px-6 py-3 text-sm font-semibold text-washi shadow-card transition hover:bg-ai-deep lg:px-8 lg:py-4 lg:text-base"
          >
            Bắt đầu
          </button>
        </div>
      ...
      {stage === "loading" && (
        <p className="mt-8 text-sumi-soft lg:text-lg">Đang tải từ vựng...</p>
      )}
      ...
      {stage === "summary" && result && (
        <div className="mt-8 rounded-xl2 border border-line/70 bg-washi/70 p-8 text-center shadow-card lg:mt-12 lg:p-12">
          {result.kind === "score" && (
            <>
              <span className="hanko-mark mx-auto flex h-16 w-16 items-center justify-center font-jp text-2xl lg:h-20 lg:w-20 lg:text-3xl">
                {result.correct === result.total ? "満点" : "終"}
              </span>
              <p className="mt-4 font-display text-2xl text-sumi lg:mt-6 lg:text-4xl">
                {result.correct} / {result.total} câu đúng
              </p>
              <p className="mt-1 text-sumi-soft lg:mt-2 lg:text-lg">
          ...
          <div className="mt-6 flex flex-wrap justify-center gap-3 lg:mt-8 lg:gap-4">
            ...
            <button
              ...
              className="rounded-full border border-line px-5 py-2.5 text-sm font-semibold text-sumi transition hover:border-ai hover:text-ai lg:px-7 lg:py-3.5 lg:text-base"
            >
              Đổi cài đặt
            </button>
          </div>
...
function Chip({
  ...
  return (
    <button
      type="button"
      onClick={onClick}
      className={`rounded-full border px-4 py-2 text-sm font-medium transition lg:px-5 lg:py-2.5 lg:text-base ${
        active
...
export default function LuyenTapPage() {
  return (
    <Suspense
      fallback={
        <div className="mx-auto max-w-3xl px-5 py-12 text-sumi-soft lg:max-w-4xl lg:px-8 lg:py-16 lg:text-lg">
          Đang tải...
        </div>
      }
    >
